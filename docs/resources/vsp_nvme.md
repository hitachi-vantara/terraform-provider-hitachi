...
page_title: "hitachi_vsp_nvme Resource - terraform-provider-hitachi"
subcategory: ""
description: |-
  Vsp NVM Subsystem Resource: Manages NVMe subsystems, including port assignments, Host NQN registrations, and Namespace mappings.
---

# hitachi_vsp_nvme (Resource)
...
- `enable_namespace_security` (Boolean) Enable or disable namespace security.
- `host_mode` (String) Host mode. Options: LINUX/IRIX, VMWARE, VMWARE_EX, AIX.
- `host_mode_options` (List of Number) List of host mode option numbers.
- `host_nqns` (Set of Object) List of Host NQNs to register. (see [below for nested schema](#nestedatt--host_nqns))
- `namespaces` (Set of Object) List of namespaces within the subsystem. (see [below for nested schema](#nestedatt--namespaces))
- `nvm_subsystem_id` (Number) NVMe ID (0-2047). If omitted, the lowest available ID is used.
- `nvm_subsystem_name` (String) NVM subsystem name (1-32 characters). Cannot start/end with space or hyphen.
- `nvm_subsystems` (Block List) NVM subsystem info. (see [below for nested schema](#nestedblock--nvm_subsystems))
- `ports` (Set of String) List of port IDs (FC-NVMe or NVMe over TCP).
- `virtual_nvm_subsystem_id` (Number) Virtual NVM subsystem ID (0-2047). Applicable for VSP 5000 series.

### Read-Only

- `id` (String) The ID of this resource.

<a id="nestedatt--host_nqns"></a>
### Nested Schema for `host_nqns`
