...
- `fault_domain_id` (String) UUID of the fault domain that should manage the volume. Mutually exclusive with `storage_controller_name`.
- `name` (String) Name of the volume. Required for create; optional for import (it will be populated during Read).
- `nick_name` (String) Nickname of the volume
- `storage_controller_name` (String) Name of the storage controller that should manage the volume. Mutually exclusive with `fault_domain_id`.
- `storage_pool` (String) Storage pool name of the storage server. Required for the create operation.
- `vosb_address` (String) The host name or the IP address (IPv4) of the VSP One SDS Block. Required for create/update and import.
