...
- `hostgroup_name` (String) HostGroup name. Required for create/update and import.
...
- `port_id` (String) Port ID. Required for create/update and import.
...
- `ldev_id` (Number) LDEV ID. Only one of ldev_id or ldev_id_hex may be specified.
- `ldev_id_hex` (String) LDEV ID in hexadecimal format. Only one of ldev_id or ldev_id_hex may be specified.
- `lun` (Number) LUN ID
...
- `ldev_id_hex` (String) Lun Path Ldev ID in hexadecimal
