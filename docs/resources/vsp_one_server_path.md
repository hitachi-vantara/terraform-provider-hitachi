...
page_title: "hitachi_vsp_one_server_path Resource - terraform-provider-hitachi"
subcategory: ""
description: |-
  Manage server paths in Hitachi storage system
---

# hitachi_vsp_one_server_path (Resource)
...
- `port_ids` (List of String) List of port IDs to associate with the server path.
- `serial` (Number) The serial number of the storage.
- `server_id` (Number) Server ID.

### Optional

- `hba_wwn` (String) Server HBA WWN. Either hba_wwn or iscsi_name must be specified, but not both.
- `iscsi_name` (String) Server iSCSI name. Either hba_wwn or iscsi_name must be specified, but not both.

### Read-Only

- `id` (String) Path ID in format 'serial-server_id-hbaWwn|iscsiName'.
