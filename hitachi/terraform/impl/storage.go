...
func GetStorageSystem(d *schema.ResourceData) (*terraformmodel.StorageSystem, error) {
	...
	if includeDetailInfo, ok := d.GetOk("include_detail_info"); ok && includeDetailInfo.(bool) {
		// When includeDetailInfo is true, pass both compressionAcceleration and version
		detailInfoType = "compressionAcceleration,version"
		log.WriteDebug("TFDebug| GetStorageSystem: include_detail_info=true, using detailInfoType: %s", detailInfoType)
	} else {
		log.WriteDebug("TFDebug| GetStorageSystem: include_detail_info=false or not set, no detailInfoType")
	}
	...
	log.WriteDebug("TFDebug| GetStorageSystem: Calling GetStorageSystemInfo with detailInfoType: '%s'", detailInfoType)
	reconStorageSystem, err := reconObj.GetStorageSystemInfo(detailInfoType)
	if err != nil {
		log.WriteDebug("TFError| error getting storage system, err: %v", err)
