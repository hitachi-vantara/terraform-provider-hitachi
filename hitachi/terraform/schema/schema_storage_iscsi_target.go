		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				...
				"ldev_id_hex": {
					Computed:    true,
					Type:        schema.TypeString,
					Description: "LDEV ID in hexadecimal",
				},
				"lun_id": {
					Computed:    true,
					Type:        schema.TypeInt,
...
var DataIscsiTargetsSchema = map[string]*schema.Schema{
	...
	"port_ids": &schema.Schema{
		...
		Optional:    true,
		Computed:    true,
		Description: "List of Port IDs that must be retrieved from the storage system server",
		Elem: &schema.Schema{
			Type: schema.TypeString,
		},
	},
	"total_iscsi_target_count": &schema.Schema{
		Type:        schema.TypeInt,
		Computed:    true,
		Description: "Total number of iSCSI Target IDs that must be retrieved from the storage system server",
	},
	// output
...
var ResourceIscsiTargetSchema = map[string]*schema.Schema{
	...
	"lun": &schema.Schema{
		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				// --- Ldev IDs / Ldev HEXs (mutually exclusive) ---
				"ldev_id": {
					Optional:     true,
					Type:         schema.TypeInt,
					Default:      -1,
