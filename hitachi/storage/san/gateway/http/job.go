...
func CheckGwyAsyncResponse(resJSONString *string) (*sanmodel.JobResponse, error) {
	...
	// Handle empty response (some invoke endpoints return 204/empty body).
	if resJSONString == nil || strings.TrimSpace(*resJSONString) == "" {
		log.WriteDebug("TFDebug| Empty response received from gateway invoke; treating as no-job-success")
...
func WaitForJobToComplete(storageSetting sanmodel.StorageDeviceSettings, jobResponse *sanmodel.JobResponse) (string, *sanmodel.JobResponse, error) {
	...
	if jobResponse != nil && jobResponse.Request.RequestURL != "" {
		...
		reqURL := jobResponse.Request.RequestURL
		if strings.Contains(reqURL, "format/invoke") || (strings.Contains(reqURL, "ldevs") && strings.Contains(reqURL, "format/invoke")) {
...
func CheckResponseAndWaitForJob(storageSetting sanmodel.StorageDeviceSettings, resJSONString *string) (*sanmodel.JobResponse, error) {
	...
	if pjobResponse.JobID == 0 {
		// job didn't start
		if pjobResponse.Error.Message != "" {
			return pjobResponse, fmt.Errorf(pjobResponse.Error.Message)
		}
		// No job and no error message -> treat as successful no-op.
		log.WriteDebug("TFDebug| No async job returned and no error message; treating as success")
