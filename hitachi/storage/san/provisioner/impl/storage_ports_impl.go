...
func (psm *sanStorageManager) GetStoragePorts(detailInfoTypes []string, portType, portAttributes string) (*[]sanmodel.StoragePort, error) {
	...
	var storagePorts *[]sangatewaymodel.StoragePort
	storagePorts, err = gatewayObj.GetStoragePorts(detailInfoTypes, portType, portAttributes)
	if err != nil {
...
// GetStoragePortsLegacy provides backward compatibility for old calls
func (psm *sanStorageManager) GetStoragePortsLegacy(detailInfoType ...string) (*[]sanmodel.StoragePort, error) {
	return psm.GetStoragePorts(detailInfoType, "", "")
}
...
func (psm *sanStorageManager) GetStoragePortByPortId(portId string) (*sanmodel.StoragePort, error) {
	...
	storagePorts, err := psm.GetStoragePorts([]string{}, "", "")
	if err != nil {
		log.WriteDebug("TFError| error in GetStoragePorts gateway call, err: %v", err)
