...
func (psm *sanStorageManager) ReconcileHostGroup(createInput *sanmodel.CreateHostGroupRequest) (*sanmodel.HostGroup, error) {
	...
	var existingHg *sanmodel.HostGroup
	var err error

	// 1. Search for existing Host Group
	if createInput.HostGroupNumber != nil {
		// PATH A: Hg number is provided - Get specific HG directly
		log.WriteDebug("HostGroupNumber provided (%d). Fetching specific HG info.", *createInput.HostGroupNumber)
		existingHg, err = psm.GetHostGroup(*createInput.PortID, *createInput.HostGroupNumber)

		// Handle case where ID is provided but doesn't exist on the array
		if err != nil || existingHg.HostGroupName == "-" {
			log.WriteInfo("HostGroup %d not found on port %s. Will attempt creation.", *createInput.HostGroupNumber, *createInput.PortID)
			existingHg = nil
		} else {
			// CHECK: Verify name matches ONLY IF name is provided in config
			if createInput.HostGroupName != nil && *createInput.HostGroupName != "" {
				if existingHg.HostGroupName != *createInput.HostGroupName {
					return nil, fmt.Errorf("hostgroup name mismatch: storage has '%s' for number %d, but config expects '%s'. ",
						existingHg.HostGroupName, *createInput.HostGroupNumber, *createInput.HostGroupName)
				}
				log.WriteDebug("HostGroup name match verified: %s", existingHg.HostGroupName)
			} else {
				// If name wasn't provided, adopt the name from the storage array
				log.WriteDebug("No HostGroupName provided in config. Adopting name from storage: %s", existingHg.HostGroupName)
				createInput.HostGroupName = &existingHg.HostGroupName
			}
		}
	} else {
		// PATH B: No hg number provided - Search by Name across all groups on the port
		log.WriteDebug("No HostGroupNumber provided. Searching by name: %s", *createInput.HostGroupName)
		allHgs, err := psm.GetAllHostGroups()
		if err == nil {
			for _, hg := range allHgs.HostGroups {
				if hg.PortID == *createInput.PortID && hg.HostGroupName == *createInput.HostGroupName {
					// Found a match by name, now fetch full details (WWNs/LUNs)
					log.WriteDebug("Found match by name. Fetching full details for HG %d", hg.HostGroupNumber)
					existingHg, err = psm.GetHostGroup(hg.PortID, hg.HostGroupNumber)
					break
				}
			}
		}
	...
	var resultHg *sanmodel.HostGroup

	if existingHg == nil {
		// CASE: Not found via either path -> CREATE
		log.WriteInfo("HostGroup %s not found. Creating new group.", *createInput.HostGroupName)
		resultHg, err = psm.createHostGroup(createInput)
		if err != nil {
			return nil, err
		}
	} else {
		// CASE: Found -> Use for Update
		log.WriteInfo("HostGroup %s found (Number: %d).", existingHg.HostGroupName, existingHg.HostGroupNumber)
		resultHg = existingHg
		// Ensure the input model has the discovered ID for the update logic
		createInput.HostGroupNumber = &existingHg.HostGroupNumber
	}

	// 2. Sync members (WWNs, LUNs, Host Mode)
	// resultHg now contains full metadata (LUN paths, WWN details) required for diffing
	finalHg, err := psm.updateHostgroup(resultHg, createInput)
	if err != nil {
		log.WriteDebug("TFError| error during hostgroup member sync: %v", err)
		return resultHg, err
	}
...
func (psm *sanStorageManager) updateHostgroup(existingHostgroup *sanmodel.HostGroup, createInput *sanmodel.CreateHostGroupRequest) (*sanmodel.HostGroup, error) {
	...
	hostGroup, err := psm.GetHostGroup(*createInput.PortID, existingHostgroup.HostGroupNumber)
	if err != nil {
		log.WriteDebug("TFError| error in GetHostGroup provisioner call, err: %v", err)
		log.WriteError(mc.GetMessage(mc.ERR_GET_HOSTGROUP_FAILED), *createInput.PortID, existingHostgroup.HostGroupNumber)
		return nil, err
	}
...
func (psm *sanStorageManager) reconcileHostGroupLuns(existingHostgroup *sanmodel.HostGroup, createInput *sanmodel.CreateHostGroupRequest) error {
	...
	for _, delete := range DeleteLU {
		if delete.Lun != nil {
			err := provObj.RemoveLdevFromHG(*createInput.PortID, existingHostgroup.HostGroupNumber, *delete.Lun)
			if err != nil {
	...
	for _, clu := range createInput.Ldevs {
		for _, elu := range existingHostgroup.LuPaths {
			if (clu.LdevId != nil) && (clu.Lun != nil) {
				log.WriteDebug("TFDebug| Comparing Create LdevID: %d with Existing LdevID: %d and Create Lun: %d with Existing Lun: %d", *clu.LdevId, elu.LdevID, *clu.Lun, elu.Lun)
